...
def ask():
    ...
    file_type = data.get('file_type')  # استلام المسار من البيانات المرسلة (إذا تم رفع صورة)
    file_path = data.get('file_path')  # استلام المسار من البيانات المرسلة (إذا تم رفع صورة)
    file_name = data.get('file_name')  # استلام المسار من البيانات المرسلة (إذا تم رفع صورة)
    ...
    if selectedCompany.get('name') == "Open Ai":
        ...
        try:
            ...
            if hasattr(response_obj, 'is_final') and response_obj.is_final:
                print("الاستجابة النهائية تم استلامها")
...
def upload_to_openai(path, api_key=""):
    ...
    if not os.path.exists(path):
        print(f"لم يتم العثور على الصورة في المسار: {path}")
        return None

    print(f"تم العثور على الصورة في المسار: {path}")  # طباعة لتأكيد المسار
    with open(path, "rb") as file:
        try:
            ...
            if 'data' in response and len(response['data']) > 0:
                ...
                print(f"تم رفع الصورة بنجاح. الرابط: {file_url}")
                return file_url
            else:
                print(f"لم تتمكن من رفع الصورة. الاستجابة: {response}")
                return None

        except openai.error.APIError as e:
            print(f"حدث خطأ في API: {str(e)}")
            return None
        except Exception as e:
...
if __name__ == '__main__':
    # app.run(port=5001)
    app.run(host="172.20.10.8", port=5001)
